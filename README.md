...
Six servo motors are controlled using PWM signals generated by three timers (TIM2, TIM3, TIM4):
- **Servo 1**: PE2 (TIM3 Channel 1)
- **Servo 2**: PE3 (TIM3 Channel 2)  
- **Servo 3**: PA0 (TIM2 Channel 1)
- **Servo 4**: PA1 (TIM2 Channel 2)
- **Servo 5**: PD12 (TIM4 Channel 1)
- **Servo 6**: PD13 (TIM4 Channel 2)

Key functions:
- **`SetServoAngle(uint8_t servoId, uint16_t angle)`**: Sets servo to specified angle (0-180°)
- **PWM Control**: Converts angle to pulse width using `PWM_MIN_PULSE` and `PWM_MAX_PULSE` constants
- **Game Integration**: Servos represent treasure locations that players can "dig" by opening them fully
